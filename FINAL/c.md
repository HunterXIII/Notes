...
# Практика 6
## Системные вызовы
Системные вызовы - это программный интерфейс между приложениями и ядром ОС. 
- `open(), read(), write()` - работа с файлами
- `fork(), exec()` - управление процессами (UNIX)
- `socket(), connect()` - сетевое взаимодействие  
  
**Программа** - набор инструкций, которые могут быть выполнены на компьютере, б*инарный модуль*  
**Процесс** - экземпляр программы во время выполнения, *запущенная программа*  
**Поток** -  одно из действий внутри процесса. Каждый поток имеет собственный виртуализированный процессор, включающий в себя стек, состояние процесса, например, регистры, а также командные указатели.  

`&` - идёт в фоне  
### unistd.h
Заголовочный файл `sys/types.h`
- `pid_t getpid()`