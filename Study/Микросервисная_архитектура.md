...
# Заметки
- Для проверки **прав доступа** самое оптимальное решение - использовать Redis с значениями в виде - **ключ**: `<chat-uuid>:<chat-uuid>` и **значение: просто int64** (64 флага различных прав) `0010101010101...`
- Есть fork для Redis-а, которые реализует многопоточность - **KeyDB** (осмысленно ставить, если сетевой интерфейс >= 10 Gb/s)
